...
public class PinnedHeaderListView extends ListView implements OnScrollListener {
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (scrollState == SCROLL_STATE_IDLE) {//设置header透明度
        	if(header!=null){
        	  header.setBackgroundColor(0xffececec);
        	}
		}else{
			if(header!=null){
				  header.setBackgroundColor(0xddececec);
			}
		}
    }
    ...
    private void ensurePinnedHeaderLayout(View header) {
    	boolean headerRequested=header.isLayoutRequested();
        if (headerRequested) {
