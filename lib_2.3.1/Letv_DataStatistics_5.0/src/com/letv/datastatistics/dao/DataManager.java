	...
	public void sendVideoInfo(final Context context, final StatisticsVideoInfo mStatisticsVideoInfo) {
		...
		ThreadPoolManager.getInstance().executeThreadWithPool(new Runnable() {
			...
			public void run() {
				...
				sb.append(DataUtils.getData(DataUtils.generateDeviceId(context)) + "&"); // auid,设备id
																							// 统计1.5版本新增
				sb.append(DataUtils.getData(mStatisticsVideoInfo.getCid()) + "&");// $3:cid,频道id
				sb.append(DataUtils.getData(mStatisticsVideoInfo.getPid()) + "_"
						+ DataUtils.getData(mStatisticsVideoInfo.getVid()) + "_"
...
//				sb.append(DataUtils.getData("5.2") + "&"); // $6：appver应用版本
				...
				sb.append(DataUtils.getData(cid) + "&");// $12:cid,频道id
				sb.append(DataUtils.getData(pid) + "_" + DataUtils.getData(vid) + "&");// $13:vinfo,视频信息pid_vid_mmsid
		...
		ThreadPoolManager.getInstance().executeThreadWithPool(new Runnable() {
			...
			public void run() {
				...
				sb.append("p1=" + DataUtils.getData(DataUtils.getTrimData(p1)) + "&");// $2
																						// p1:一级产品线代码
//				DataUtils.getData(DataUtils.getTrimData(p1 + p2))																 为从0开始的数字
				sb.append("p2=03" +  "&");// $3
																							// p2:二级产品线代码
																							// 已一级产品线代码为前缀，接下来从0开始
				sb.append("p3=031" + "&");// $4 p3 三级产品线代码 平台自定义
				...
				sb.append("cid=" + DataUtils.getData(cid) + "&");// $8
																	// cid:视频频道ID
																	// 全业务线统一
																	// 和大媒资一致
				sb.append("pid=" + DataUtils.getData(pid) + "&");// $9 pid:专辑ID
																	// 如果不是专辑 用
																	// - 替代
				sb.append("vid=" + DataUtils.getData(vid) + "&");// $10 vid:
																	// 视频ID
	...
	public void sendPlayInfo(final Context context, final String p1, final String p2, final String ac,
			...
			final String weid, final String pcode, final int ilu, final String ch,final String lc,final String lid) {
		ThreadPoolManager.getInstance().executeThreadWithPool(new Runnable() {
			@Override
			public void run() {
...
//				sb.append("p2=" + DataUtils.getData(DataUtils.getTrimData(p1 + p2)) + "&");// $3
				...
				sb.append("lc=" +DataUtils.getData(lc)+ "&"); // $10 lc:(letv cookie)
											// 是用来唯一标识用户的，用户即使用不同的浏览器，
											// lc都是相同的，可以参考flash cookie
				sb.append("lid=" +DataUtils.getData(lid)+ "&"); 
				...
				sb.append("cid=" + DataUtils.getData(cid) + "&");// $13
																	// cid:视频频道ID
																	// 全业务线统一
																	// 和大媒资一致
				sb.append("pid=" + DataUtils.getData(pid) + "&");// $14 pid:专辑ID
																	// 如果不是专辑 用
																	// - 替代
				sb.append("vid=" + DataUtils.getData(vid) + "&");// $15 vid:
																	// 视频ID
				sb.append("vlen=" + DataUtils.getData(vlen) + "&");// $16 vlen:
																	// 视频时长
																	// 以秒为单位
				sb.append("ch="+ DataUtils.getData(ch) + "&");// $17 ch:渠道
				sb.append("zid=-"+"&");// $17 ch:渠道
				...
				sb.append("py=" +  URLEncoder.encode(DataUtils.getData(py)) + "&");// $24
																											// py:
																											// Property
																											// 播放属性
...
//				sb.append("p2=" + DataUtils.getData(DataUtils.getTrimData(p1 + p2)) + "&");// $3
				...
				sb.append("cid=" + DataUtils.getData(cid) + "&");// $5
																	// cid:视频频道ID
																	// 全业务线统一
																	// 和大媒资一致
				sb.append("pid=" + DataUtils.getData(pid) + "&");// $6 pid:专辑ID
																	// 如果不是专辑 用
																	// - 替代
				sb.append("vid=" + DataUtils.getData(vid) + "&");// $7 vid: 视频ID
	 ...
	 *            ip地址
	 */
	public void sendEnvInfo(final Context context, final String p1, final String p2, final String ip, final String xh,final boolean isplayer) {
		ThreadPoolManager.getInstance().executeThreadWithPool(new Runnable() {
			@Override
			public void run() {
				if (DataUtils.getAvailableNetWorkInfo(context) == null) {
...
//				sb.append("p2=" + DataUtils.getData(DataUtils.getTrimData(p1 + p2)) + "&");// $3
				...
				sb.append("lc=-" + "&"); // $5 lc:(letv cookie)
											...
											// lc都是相同的，可以参考flash cookie
				if(isplayer){
					sb.append("uuid=-"+ "&"); // $6在播放器里面传-，在播放器外要传uuid
					// uuid:(did)
				}else{
				...
				sb.append("ep=" + URLEncoder.encode(DataUtils.getData("model="+DataUtils.getDeviceName())) + "&");																							// url编码
				...
				sb.append("ro=" + DataUtils.getDataEmpty(DataUtils.getNewResolution(context)) + "&"); // 15:
																										// resolution:设备分辨率
				sb.append("zid=-" + "&");// $16 br:Browser浏览器名称 String																						// 1024_768
				sb.append("br=chrome" + "&");// $16 br:Browser浏览器名称 String
												// 由数据部维护字典,（见附表三） [选填]
				sb.append("r=" + System.currentTimeMillis()); // $17: 随机数
				if (DataStatistics.getInstance().isDebug()) {
...
//				sb.append("p2=" + DataUtils.getData(DataUtils.getTrimData(p1 + p2)) + "&");// $3
				...
				sb.append("pp=" + URLEncoder.encode(DataUtils.getData(pp)) + "&"); // $15:pp:广告属性
																					...
																					// &，要进行URL编码
																					// [选填]
				sb.append("cid=" + DataUtils.getData(cid) + "&"); // $5 cid
