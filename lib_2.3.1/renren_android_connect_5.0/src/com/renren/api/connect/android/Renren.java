...
import com.renren.api.connect.android.view.RenrenLoginActivity;
...
public class Renren implements Parcelable {
	...
	public void authorize(Activity activity, final RenrenAuthListener listener, int width, int height) {
		this.authorize(activity, (String[])null, listener, width, height);
	}
	
	public void authorizeFull(Activity activity, final RenrenAuthListener listener) {
		this.authorizeFull(activity, (String[])null, listener);
	}
	...
	public void authorize(Activity activity, String[] permissions,
			final RenrenAuthListener listener, int width, int height) {
		if (this.isSessionKeyValid()) {
			listener.onComplete(new Bundle());
			return;
		}
	...
	public void authorizeFull(Activity activity, String[] permissions,
			final RenrenAuthListener listener) {
		if (this.isSessionKeyValid()) {
			listener.onComplete(new Bundle());
			return;
		}
		RenrenDialogListener rdl = RenrenListenerFactory.genUserAgentFlowRenrenDialogListener(this, listener,
						DEFAULT_REDIRECT_URI);
		this.authorizeFull(activity, permissions, rdl, DEFAULT_REDIRECT_URI,
				"token");
	}
	...
	private void authorize(Activity activity, String[] permissions,
			final RenrenDialogListener listener, String redirectUrl,
			String responseType, int width, int height) {
		// 调用CookieManager.getInstance之前
		// 必须先调用CookieSyncManager.createInstance
		CookieSyncManager.createInstance(activity);
		...
		if (activity.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			...
			new RenrenDialog(activity, url, listener, width, height).show();
		}
	}
	
	
	private void authorizeFull(Activity activity, String[] permissions,
			final RenrenDialogListener listener, String redirectUrl,
			String responseType) {
		// 调用CookieManager.getInstance之前
		// 必须先调用CookieSyncManager.createInstance
		...
		if (activity.checkCallingOrSelfPermission(Manifest.permission.INTERNET) != PackageManager.PERMISSION_GRANTED) {
			...
			RenrenLoginActivity.Lanuch(activity, url, listener);
		}
	}
	...
	public void authorize(Activity activity, String[] permissions,
			final RenrenAuthListener listener, int requestCode, int width, int height) {
		//若开发者提供的权限列表为空，则使用默认权限列表
		if(permissions == null) {
			permissions = DEFAULT_PERMISSIONS;
		}
		this.mSso.authorize(activity, permissions, listener, requestCode, width, height);
	}
