...
import com.letv.android.slidingmenu.lib.SlidingMenu.CanvasTransformer;
...
public class CustomViewBehind extends ViewGroup {
	...
	private int mLWidthOffset,mRWidthOffset;
	...
	public void setLeftWidthOffset(int i) {
		mLWidthOffset = i;
		requestLayout();
	}
	...
	public int getLeftBehindWidth() {
		return mContent.getWidth();
	}
	
	public int getRightBehindWidth() {
		return mSecondaryContent.getWidth();
	}

	public void setContent(View v) {
		if (mContent != null){
			ViewGroup vs=(ViewGroup) mContent.getParent();
			 vs.removeAllViews();
			removeView(mContent);
		}
		mContent = v;
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		if(mContent!=null)
		mContent.layout(0, 0, width-mLWidthOffset, height);
		if (mSecondaryContent != null){
//			mSecondaryContent.layout(-mLWidthOffset, 0, width-mRWidthOffset-mLWidthOffset, height);
//			mSecondaryContent.layout(mRWidthOffset-mLWidthOffset, 0, width-mLWidthOffset, height);
//			mSecondaryContent.layout(mLWidthOffset - mRWidthOffset, 0, width+mLWidthOffset-mRWidthOffset*2, height);
			mSecondaryContent.layout(0, 0, width-mRWidthOffset, height);
//			mSecondaryContent.layout(mRWidthOffset, 0, width, height);
		}
	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		final int contentWidth = getChildMeasureSpec(widthMeasureSpec, 0, width-mLWidthOffset);
		final int contentHeight = getChildMeasureSpec(heightMeasureSpec, 0, height);
		if(mContent!=null)
		mContent.measure(contentWidth, contentHeight);
		if (mSecondaryContent != null)
			mSecondaryContent.measure(getChildMeasureSpec(widthMeasureSpec, 0, width-mRWidthOffset), contentHeight);
	}
	...
	public void scrollBehindTo(View content, int x, int y) {
		...
		if (mMode == SlidingMenu.LEFT) {
			...
			scrollTo((int)((x + getLeftBehindWidth())*mScrollScale), y);
		} else if (mMode == SlidingMenu.RIGHT) {
			if (x <= content.getLeft()) vis = View.INVISIBLE;
			scrollTo((int)(getLeftBehindWidth() - getWidth() + 
					(x-getLeftBehindWidth())*mScrollScale), y);
			...
			if (x <= content.getLeft()) {
				...
				scrollTo((int)(getRightBehindWidth() - getWidth() + 
						(x-getRightBehindWidth())*mScrollScale), y);	
//				scrollTo((int)(getLeftBehindWidth() - getWidth() + 
//						(x-getLeftBehindWidth())*mScrollScale), y);
                        if(x-getRightBehindWidth()==0){
	...
	public int getMenuLeft(View content, int page) {
		if (mMode == SlidingMenu.LEFT) {
			switch (page) {
			case 0:
				return content.getLeft() - getLeftBehindWidth();
			case 2:
...
//				return content.getLeft() + getLeftBehindWidth();
	...
	public void drawFade(View content, Canvas canvas, float openPercent) {
		...
		int right = 0;
		if (mMode == SlidingMenu.LEFT) {
			left = content.getLeft() - getLeftBehindWidth();
			right = content.getLeft();
		} else if (mMode == SlidingMenu.RIGHT) {
			left = content.getRight();
			right = content.getRight() + getLeftBehindWidth();			
		} else if (mMode == SlidingMenu.LEFT_RIGHT) {
			left = content.getLeft() - getLeftBehindWidth();
			right = content.getLeft();
