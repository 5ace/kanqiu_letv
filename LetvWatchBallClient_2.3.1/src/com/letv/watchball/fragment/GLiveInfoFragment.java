...
//	private ProgressBar autoRefreshBar;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
			    ...
			    try {
			    	root = (ViewGroup) inflater.inflate(R.layout.fragment_home_live,null, false);
			    } catch (InflateException e) {
			        /* map is already there, just return view as it is */
			    }
			
			mPullToRefreshListViewHeader = (PullToRefreshListViewHeader) root.findViewById(R.id.fragment_home_live);
			
			mPullToRefreshListViewHeader.getRefreshableView().setFadingEdgeLength(0);
			mPullToRefreshListViewHeader.getRefreshableView().setDivider(null);
//			mPullToRefreshListViewHeader.getRefreshableView().setDivider(new ColorDrawable(getResources().getColor(R.color.letv_color_ffd5d5d5)));
			
//			autoRefreshBar = (ProgressBar) root.findViewById(R.id.auto_refrsh_progress);
			
			refreshBar = (MyProgressBar) root.findViewById(R.id.refreshBar);
			refresh = (ImageView) root.findViewById(R.id.refresh);
			refreshBar.setProgressBarCallBack(new MyProgressBar.ProgressBarCallBack() {
...
//		}
	...
	private class AutoRefreshHandler extends Handler{
		...
		public void handleMessage(Message msg) {
			...
			switch (msg.what) {
			case 1:
				if(progressbar.getPositon() == 359 && null != root && root.getVisibility()==View.VISIBLE){
	...
	private void init(){
		...
		mPullToRefreshListViewHeader.setOnRefreshListener(new OnRefreshListener2<PinnedHeaderListView>() {
...
//				mHomeFragmentLsn.loadRightFragmentData();
		...
		mPullToRefreshListViewHeader.setOnScrollListener(new OnScrollListener() {
			
			@Override
			public void onScrollStateChanged(AbsListView view, int scrollState) {
                if (null != mPullToRefreshListViewHeader){
                    mPullToRefreshListViewHeader.getRefreshableView().onScrollStateChanged(view, scrollState);
                }
			...
			public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
                if (null != mPullToRefreshListViewHeader){
				    mPullToRefreshListViewHeader.getRefreshableView().onScroll(view, firstVisibleItem, visibleItemCount, totalItemCount);
                }
	...
	private void loadFirstPage(boolean dialog,boolean needSetSelection){
		new RequestMatchList(getActivity(),prePageIndex, dialog,needSetSelection,null).start();
	}
	/**
	 * 加载上一页数据
	 */
	private void loadPrePage(){
		prePageIndex--;
		new RequestMatchList(getActivity(), prePageIndex, false ,true,new Runnable() {
...
//				prePageIndex++;
	...
	private void loadNextPage(){
		nextPageIndex++;
		new RequestMatchList(getActivity(), nextPageIndex, false ,false,new Runnable() {
	...
	private class RequestMatchList extends LetvHttpAsyncTask<LiveList>{
		...
		public RequestMatchList(Context context,int pageIndex,boolean dialog,boolean needSetSelection,Runnable errorCb) {
		...
		public LiveList loadLocalData() {
			try {
				...
				if (pageIndex == 0 && needSetSelection) {
					// first page
					LocalCacheBean bean = LetvCacheDataHandler.readHomeLiveData();
					if (bean != null) {
						LiveList result = null;
						LetvGsonParser<LiveList> albumNewParse = new LetvGsonParser<LiveList>(0, LiveList.class);
						result = albumNewParse.initialParse(bean.getCacheData());
						String fristdate=new SimpleDateFormat("yyyy-MM-dd", Locale.CHINESE).format(Calendar.getInstance().getTime());
...
//						}
						...
						for (int i = 0; i < result.body.length; i++) {
							...
							if(LoacalDateSelection){
								System.out.println("mLiveInfos.length="+mLiveInfos.length);
							...
							for (int j = 0; j < mLiveInfos.length; j++) {
								...
								// 设置listview起始位置
								if ( LoacalDateSelection==false&&mLiveInfos[j].status == 2)  {
										selection++;
								}
								childrens.add(game);
							}
...
//							adapter.listParent.add(i + firstPageIndex, result.body[i].date);
//							adapter.listChild.add(i + firstPageIndex, childrens);
		...
		public LetvDataHull<LiveList> doInBackground() {
			LetvDataHull<LiveList> dataHull = LetvHttpApi.requestLiveinfos(pageIndex+"", new LetvGsonParser<LiveList>(0, LiveList.class));
			...
			if (pageIndex==0) {
				...
				if (adapter.listParent.size() > firstPageIndex && firstPageSize > 0) {
...
//				mPullToRefreshListViewHeader.setVisibility(View.GONE);
	...
	/*
	private class HomePageNotyfy{
		private static final int FIRET_PAGE = 0;
		private static final int PRE_PAGE = 1;
		private static final int NEXT_PAGE = 2;
		private static final int ERROR = 3;
		public void notify(int code,LiveList result) {
			switch (code) {
			case FIRET_PAGE:
