...
public class UpdateDownloadAsyncTask extends LetvAsyncTask<Integer, Void>{
	...
	private ProgressDialog mProgressDialog = null;
		...
		public void handleMessage(android.os.Message msg) {
			...
			case 0 :
                        if (forseUpdate){
                              ...
                              updateNotification.setLatestEventInfo(activity, notiTitle, progress+"%", updatePendingIntent);
			...
			case 1 :
                        if (forseUpdate){
                              ...
                              updatePendingIntent = PendingIntent.getActivity(activity, 0, intent, 0);

                              updateNotification.defaults = Notification.DEFAULT_SOUND;//铃声提醒
                              updateNotification.setLatestEventInfo(activity, "看球", "下载完成,点击安装。", updatePendingIntent);
                              updateNotificationManager.notify(NOTIFY_ID, updateNotification);
                        }
      ...
      public UpdateDownloadAsyncTask(Activity activity ,String url , String newV,String upType) throws UpdataAppException{
    ...
    public UpdateDownloadAsyncTask(SettingsActivity settingsActivity, UpgradeInfo result) throws UpdataAppException {
        this(settingsActivity,result.getUrl(),result.getV(),result.getUptype());
    }

      public UpdateDownloadAsyncTask(MainActivity mainActivity, String url, String v, String uptype, Handler mHandler) throws UpdataAppException {
	...
	protected Void doInBackground() {
		...
		if(!Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)){
			path = LetvApplication.getInstance().getDir("updata", Context.MODE_WORLD_WRITEABLE | Context.MODE_WORLD_READABLE | Context.MODE_PRIVATE).getPath();
		}else{
...
//			outputStream.seek(startPosition);
	...
	protected void onPreExecute() {
		super.onPreExecute();
            ...
            forseUpdate = !TextUtils.isEmpty(upType) && upType.equals(UpgradeInfo.UPTYPE_FORCE);
        if (forseUpdate){
        	if(null != mProgressDialog){
        		mProgressDialog.dismiss();
		...
		mProgressDialog.setMessage(TextUtil.text(R.string.updatedownloadasynctask_download, newV));
		mProgressDialog.show();
        } else {
              updateNotificationManager = (NotificationManager)activity.getSystemService(Context.NOTIFICATION_SERVICE);
              ...
              updateNotification.setLatestEventInfo(activity,notiTitle,"0%", updatePendingIntent);
              updateNotificationManager.notify(NOTIFY_ID , updateNotification);
        }
	}
