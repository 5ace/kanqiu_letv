...
public class LetvTencentQzoneShare {
	...
	public void addShare(final ShareAlbum album, final int order, final int vid) {
		onTencentQZResult = new onTencentQZResult() {
			...
			public void onTencentQZResult_back(String texturl) {
				...
				replace_url = LetvShareControl.getInstance().getShare()
						.getVideo_url()
						.replace("{aid}", album.getShare_id() + "");
				replace_url = replace_url.replace("{index}", "1");
				replace_url = replace_url.replace("{vid}", vid + "");
				BaseApiListener mBaseApiListener = new BaseApiListener(
						"add_share", true, mTencent);
				...
				mTencent.requestAsync(Constants.GRAPH_ADD_SHARE, parmas,
						Constants.HTTP_POST, mBaseApiListener, null);
	 ...
	 * 登录
	 * */
	public void login(final Context context, final ShareAlbum album,
			final int order, final int vid) {
		mxcontext = context;
		if (mTencent == null) {
			mTencent = Tencent.createInstance(
					ShareConstant.TencentQzone.mAppid, context);
		}
		addShare(album, order, vid);
		if (satisfyConditions() == ShareConstant.BindState.BIND
				&& context instanceof BasePlayActivity) {
			mTencent.setAccessToken(mAccessToken, mExpires_in + "");
			mTencent.setOpenId(mOpenId);

			SharePageActivity.launch(context, 3, album.getShare_AlbumName(),
					album.getIcon(), album.getShare_id(), album.getType(),
					album.getCid(), album.getYear(), album.getDirector(),
					album.getActor(), album.getTimeLength(), order, vid, false, "");
		...
		listener = new IUiListener() {
			...
			public void onComplete(JSONObject arg0) {
				...
				try {
					...
					mExpires_in = System.currentTimeMillis()
							+ Long.parseLong(arg0.getString("expires_in"))
							* 1000;
					saveLogin(context);
					if (satisfyConditions() == ShareConstant.BindState.BIND
							&& context instanceof BasePlayActivity) {
						SharePageActivity.launch(context, 3,
		...
		if (context != null && mTencent != null) {
			mTencent.login((Activity) context,
					ShareConstant.TencentQzone.scope, listener);
		}
	}
	...
	public static void logout(Activity context) {
		...
		mOpenId = null;
		mExpires_in = 0;
		SharedPreferences preferences = context.getSharedPreferences("qzone",
	...
	private static int satisfyConditions() {
		if (mAccessToken != null && ShareConstant.TencentQzone.mAppid != null
				&& mOpenId != null && !mAccessToken.equals("")
				&& !ShareConstant.TencentQzone.mAppid.equals("")
				&& !mOpenId.equals("")) {
	...
	public class BaseApiListener implements IRequestListener {
		...
		protected void doComplete(JSONObject response, Object state) {
			try {
				...
				if (ret == 100030) {
					if (mNeedReAuth) {
						Runnable r = new Runnable() {
							public void run() {
								if (mxcontext != null) {
									mxTencent.reAuth((Activity) mxcontext,
											mScope, listener);
								}
				...
				notification.setLatestEventInfo((Activity) mxcontext, null,
						null, contentIntent);
