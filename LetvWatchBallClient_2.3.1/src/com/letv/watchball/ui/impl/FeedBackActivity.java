...
public class FeedBackActivity extends LetvBaseActivity implements OnClickListener {
...
//		getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_STATE_VISIBLE);
	...
	private void findView() {
		...
		//判断用户是否登录
		/**
		 * 已登录则，则带上登录token,及跳转地址http://q.letv.com
		 * 没有登录则直接跳转到http://q.letv.com
		 * */
		StringBuffer urlSb = new StringBuffer("http://sso.letv.com/user/setUserStatus");
		String token = PreferencesManager.getInstance().getSso_tk();
		if(!token.equals("")){
			urlSb.append("?tk=").append(token)
 			.append("&from=").append("sports")
			.append("&next_action=").append("http://q.letv.com/m/letv_app-kanqiu_android");
			feedback_webview.loadUrl(urlSb.toString());
		}else{
			feedback_webview.loadUrl("http://q.letv.com/m/letv_app-kanqiu_android");
		}
...
//		new RequestFeedBackLoginInfo(FeedBackActivity.this,urlSb.toString()).start();
//		createHead();
//		feedback_sex = this.getResources().getStringArray(R.array.feedback_sex);
//		feedback_age = this.getResources().getStringArray(R.array.feedback_age);
//		feedback_submit = (Button) findViewById(R.id.feedback_submit);
//		feedback_submit_unenable = (Button) findViewById(R.id.feedback_submit_unenable);
...
//		spinner_sex.setListener(new LetvSpinner.OnLetvSpinnerSelectedListener(){
//			@Override
//			public void onSelected(int pos) {
//				sexPos = pos+1;
//			}
//		});
//		spinner_age.setListener(new LetvSpinner.OnLetvSpinnerSelectedListener(){
//			@Override
//			public void onSelected(int pos) {
//				agePos = pos+1;
//			}
...
//
//		@Override
//		public void onPostExecute(int updateId, com.letv.watchball.bean.State result) {
//			if (result.isSucceed()) {
