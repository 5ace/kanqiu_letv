...
public class VideoFileParser extends LetvMobileParser<VideoFile>{
	...
	public VideoFile parse(JSONObject data) throws JSONException {
		...
		if(has(data, MP4_350)){
			videoFile.setMp4_350(new VideoSchedulingAddressParser(needPay).parse(getJSONObject(data, MP4_350)));
		}
		
		if(has(data, MP4_1000)){
			videoFile.setMp4_1000(new VideoSchedulingAddressParser(needPay).parse(getJSONObject(data, MP4_1000)));
		}
		
		if(has(data, MP4_1300)){
			videoFile.setMp4_1300(new VideoSchedulingAddressParser(needPay).parse(getJSONObject(data, MP4_1300)));
		}
		
		if(has(data, MP4_800_DB)){
			videoFile.setMp4_800_db(new VideoSchedulingAddressParser(needPay).parse(getJSONObject(data, MP4_800_DB)));
		}
		
		if(has(data, MP4_1300_DB)){
			videoFile.setMp4_1300_db(new VideoSchedulingAddressParser(needPay).parse(getJSONObject(data, MP4_1300_DB)));
		}
		
		if(has(data, MP4_720P_DB)){
			videoFile.setMp4_720p_db(new VideoSchedulingAddressParser(needPay).parse(getJSONObject(data, MP4_720P_DB)));
		}
		
		if(has(data, MP4_1080P6M_DB)){
			videoFile.setMp4_1080p6m_db(new VideoSchedulingAddressParser(needPay).parse(getJSONObject(data, MP4_1080P6M_DB)));
		}
