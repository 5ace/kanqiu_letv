...
public class LetvLiveReceiver extends BroadcastReceiver{
	...
	public void onReceive(Context context, Intent in) {
		...
		if(PreferencesManager.getInstance().getGameStartRemind() > 0 && LetvUtil.sleepAlarm()) {
			ArrayList<PushSubscribeGame> list = DBManager.getInstance().getSubscribeGameTrace().getCurrentTrace();
			if(list != null && list.size() > 0) {
				for (int i = 0; i < list.size(); i++) {

                              String liveNtificationGameId = PreferencesManager.getInstance().getLiveNtificationGameId();
                              PushSubscribeGame mPushSubscribeGame = list.get(i);
                              DBManager.getInstance().getSubscribeGameTrace().updateNotity(mPushSubscribeGame.id,true);

                              if (mPushSubscribeGame.id.equals(liveNtificationGameId)){
                                    continue;
                              }

                              String notiTitle = context.getResources().getString(R.string.letvpushservice_live_title);
                              String notiContent = mPushSubscribeGame.home+"VS"+mPushSubscribeGame.guest+"  "+mPushSubscribeGame.playTime;
					Intent intent= new Intent(context , MainActivity.class);
					intent.setAction(PushNotificationReceiver.NOTIFY);
					int codeToInt = LetvUtil.codeToInt(mPushSubscribeGame.id);
			    	      PendingIntent pendingIntent = PendingIntent.getActivity(context, codeToInt, intent, PendingIntent.FLAG_UPDATE_CURRENT);
...
//					notification(context, notiTitle, notiContent);
                              PreferencesManager.getInstance().setLiveNtificationGameId(mPushSubscribeGame.id);

					LetvLogTool.getInstance().log("\r\n\r\n\r\n"+"time:"+LetvUtil.timeFormat(System.currentTimeMillis())+"\r\n");
					LetvLogTool.getInstance().log("notifycationId:"+codeToInt+"notiTitle:"+notiTitle+"\r\n"+"notiContent:"+notiContent);
				}
			}
...
//	public static void notification(Context context,String notiTitle,String notiContent){
//		LetvLogTool.getInstance().log("\r\n\r\n\r\n"+"time:"+LetvUtil.timeFormat(System.currentTimeMillis())+"\r\n");
...
////		System.out.println("mPushSubscribeGame.getCode():" + mPushSubscribeGame.getCode());
////		System.out.println("mPushSubscribeGame.getProgramName():" + mPushSubscribeGame.getProgramName());
//		notificationManager.notify(codeToInt , notification);
//	}
