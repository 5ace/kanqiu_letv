...
public class LetvOpenIDOAuthLoginActivity extends LetvBaseActivity implements
		...
		public void onPageStarted(WebView view, String url, Bitmap favicon) {
			try {
				...
				if (path.contains("callbackdata")) {
					view.stopLoading();
					new RequestLoginTask(LetvOpenIDOAuthLoginActivity.this, url)
							.start();
				}
			} catch (Exception e) {
				e.printStackTrace();
	...
	private class RequestLoginTask extends LetvHttpAsyncTask<User> {
		...
		public LetvDataHull<User> doInBackground() {
			LetvDataHull<User> dataHull = LetvHttpApi.openIDOAuthLogin(0, url,
					new OpenUserParser());
		...
		public void onPostExecute(int updateId, User result) {
			...
			// LoginMainActivity.launchBySrc(LetvOpenIDOAuthLoginActivity.this,
			// src);
			// }
