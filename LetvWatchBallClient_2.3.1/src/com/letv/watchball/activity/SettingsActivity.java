...
public class SettingsActivity extends LetvBaseActivity implements OnClickListener ,Icloseable{
	...
	protected void onCreate(Bundle savedInstanceState) {
        ...
        mSwitchView.setSelection(PreferencesManager.getInstance().isGameResultRemind() ? 0 : 1);
        ...
        mSleepSwitchView.setSelection(PreferencesManager.getInstance().isSleepRemind() ? 0 : 1);
        ...
        if (LetvUtil.hasNet()) {
            DataStatusInfo mDataStatusInfo = LetvApplication.getInstance().getDataStatusInfo();

            if (mDataStatusInfo != null) {
                UpgradeInfo mUpgradeInfo = mDataStatusInfo.getUpgradeInfo();

                if (mUpgradeInfo != null && UpgradeInfo.UPGRADE_YES.equals(mUpgradeInfo.getUpgrade())) {
	...
	private void setGameStartRemind() {
		...
		int[] gameRemindArrInt = getResources().getIntArray(R.array.game_remind_arr_int);
		int index = 0;
		...
		((TextView) findViewById(R.id.seting_item_game_start_tv)).setText(getResources().getStringArray(R.array.game_remind_arr_str)[index]);
		if(index == gameRemindArrInt.length - 1){
			((TextView) findViewById(R.id.seting_item_game_start_tv)).setText(getResources().getStringArray(R.array.game_remind_arr_str)[index].subSequence(0, 2));
		}
	}
	...
	private void setPlayHd() {
		...
		((TextView) findViewById(R.id.settings_play_hd)).setText(playHdArr[(playHd?0:1)]);
	}
	...
	public void onClick(View v) {
		...
		case R.id.settins_item_hd:
			// 清晰度设置
			startActivityForResult(new Intent(SettingsActivity.this, SettingsHdActivity.class), REQUEST_CODE_PLAY_HD);
			break;
		case R.id.seting_item_game_start_ctrl:
			// 开赛提醒设置
			startActivityForResult(new Intent(SettingsActivity.this, SettingsGameRemindActivity.class), REQUEST_CODE_GAME_START_REMIND);
			break;
		...
		if (LetvUtil.hasNet()) {
			...
			if (mDataStatusInfo != null) {
				...
				if (mUpgradeInfo != null && UpgradeInfo.UPGRADE_YES.equals(mUpgradeInfo.getUpgrade())) {
					...
					PreferencesManager.getInstance().setIsNeedUpdate(false);
					UIs.callDialogMsgPositiveButton(SettingsActivity.this, -1, R.string.dialog_messge_update_is_newest, null);
				}
			} else {
				new RequestDataStatusInfo(this).start();
			}
            ...
            UIs.call(this,result.getTitle(), result.getMsg(),
                    ...
                    new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            try {
                                new UpdateDownloadAsyncTask(SettingsActivity.this, result).execute();
...
//				String.format(getResources().getString(R.string.update_dialog_has_new_version), result.getV()),
//
//				new DialogInterface.OnClickListener() {
//					@Override
//					public void onClick(DialogInterface dialog, int which) {
...
//								asyncTask = new DownloadAsyncTask(SettingsActivity.this, result.getUrl(), result.getV());
//								asyncTask.execute();
//							} else {
//								UIs.notifyShortNormal(SettingsActivity.this, R.string.already_updata);
//							}
...
//				}, R.string.update_exit);
    ...
    private class RequestDataStatusInfo extends LetvHttpAsyncTask<LetvBaseBean> {
		...
		public LetvDataHull<LetvBaseBean> doInBackground() {
			try {
				DataStatusInfo result = DataStatistics.getInstance().getDataStatusInfo(SettingsActivity.this, LetvUtil.getPcode());
				if (result != null) {
					...
					if (result.getRecommendInfos() != null) {
						...
						for (String key : result.getRecommendInfos().keySet()) {
							...
							RecommendInfo mRecommendInfo = result.getRecommendInfos().get(key);
							...
							} else if (RecommendInfo.RECOMMEND_KEY_LIVE.equals(key)) {
								if (mRecommendInfo.isOpen()) {
									if (mRecommendInfo.getNum() > 0) {
							...
							} else if (RecommendInfo.RECOMMEND_KEY_DOWNLOAD.equals(key)) {
								if (mRecommendInfo.isOpen()) {
									if (mRecommendInfo.getNum() > 0) {
							...
							} else if (RecommendInfo.RECOMMEND_KEY_SETTING.equals(key)) {
								if (mRecommendInfo.isOpen()) {
									if (mRecommendInfo.getNum() > 0) {
		...
		public void netErr(int updateId, String errMsg) {
			...
			if (mDataStatusInfo != null) {
				...
				if (mUpgradeInfo != null && UpgradeInfo.UPGRADE_YES.equals(mUpgradeInfo.getUpgrade())) {
					...
					UIs.callDialogMsgPositiveButton(SettingsActivity.this, -1, R.string.update_dialog_newest_already, null);
