 ...
 * </ul>
 */
public class CircleGalleryIndicator extends View implements AnimationListener {
	...
	private LetvGallery adpterView;
	private int totle;
	 ...
	 * @param attrs
	 */
	public CircleGalleryIndicator(Context context, AttributeSet attrs) {
		super(context, attrs);
		...
		mCentered = a.getBoolean(R.styleable.CircleFlowIndicator_ccentered, false);
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawCircle(leftPadding + radius + circleSeparation * curPos + centeringOffset, getPaddingTop() + radius, radius + activeRadius, mPaintActive);
	}
	 ...
	 * .widget.ViewFlow)
	 */
	public void setViewFlow(LetvGallery view) {
//		resetTimer();
		adpterView = view;
		flowWidth = adpterView.getWidth();
//		invalidate();
	}
	...
	public void setTotle(final int totle) {
		...
		this.totle = totle;
		if(totle > 0){
			adpterView.addSelectedListener(new OnItemSelectedListener() {

				@Override
				public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
					curPos = position % totle ;
					invalidate();
				}
	...
	public void onScrolled(int h, int v, int oldh, int oldv) {
		...
		resetTimer();
		flowWidth = adpterView.getWidth();
		if (totle * flowWidth != 0) {
			currentScroll = h % (totle * flowWidth);
		} else {
			currentScroll = h;
		}
		invalidate();
