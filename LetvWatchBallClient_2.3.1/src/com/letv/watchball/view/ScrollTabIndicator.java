...
public class ScrollTabIndicator extends HorizontalScrollView implements
	...
	private void initTabs() {
		...
		mCursor.setLayoutParams(new LayoutParams(width,
		...
		mAdapter.updateView(mTabIndicator.getChildAt(currentPos), null,
				currentPos, 0);
	}
	...
	protected void calculateOffset(int index, int silderW) {
		...
		mCursor.post(new Runnable() {
			...
			@Override
			public void run() {
				LinearLayout.LayoutParams params = (LinearLayout.LayoutParams) mCursor
						.getLayoutParams();
	...
	public void onPageScrolled(final int position, final float positionOffset,
		...
		mCursor.post(new Runnable() {
			...
			public void run() {
				 ...
				 * 从该方法的返回的参数来看,变化过程如下: 1.position: 0 -> 1 positionOffset: 0 ->
				 * 0.99xxx -> 0 2.position: 1 -> 0 positionOffset: 0 -> 0.99xxx
				 * -> 0 从上面的变化过程来看,position+positionOffset就是当前页面滑动的距离,与方向无关
				 * 这个变化过程,可以看成三个pager不动,有一个大的滑块从第一个到第二个平滑的移动
				 */
				params.leftMargin = (int) ((position + positionOffset) * mCursor
						.getWidth());
